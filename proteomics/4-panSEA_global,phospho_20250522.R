  ...
  for (i in 1:length(omics)) {
    ...
    if (names(method.data)[k] != "DIA_&_TMT") {
      ...
      for (j in 1:length(temp.phenos)) {
        ...
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), ".pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        
        MSnSet.utils::plot_pca(pca.data, phenotype = temp.phenos[j], label = "batch") + ggtitle(paste(names(method.data)[k], names(omics)[i], "PCA"))
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_batchLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        ...
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_flowLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        ...
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_cellLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        
      } 

      pca.data <- correct_batch_effect_NA(pca.data, "batch", par.prior = T)
      write.table(exprs(pca.data),
      ...
      for (j in 1:length(temp.phenos)) {
        MSnSet.utils::plot_pca(pca.data, phenotype = temp.phenos[j], label = "patient") + ggtitle(paste(names(method.data)[k], names(omics)[i], "PCA"))
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_batchCorrected.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        
        MSnSet.utils::plot_pca(pca.data, phenotype = temp.phenos[j], label = "batch") + ggtitle(paste(names(method.data)[k], names(omics)[i], "PCA"))
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_batchCorrected_batchLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
        
        MSnSet.utils::plot_pca(pca.data, phenotype = temp.phenos[j], label = "Sort Type") + ggtitle(paste(names(method.data)[k], names(omics)[i], "PCA"))
        ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_batchCorrected_flowLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
      ...
      # if ("batch" %in% phenos){ # after fixed meta ids: Error in ComBat.NA(exprs(m), batch, mod = mod, par.prior = par.prior,  : 
      ...
      # 
      #   MSnSet.utils::plot_pca(pca.data, phenotype = temp.phenos[j], label = "Sort Type") + ggtitle(paste(names(method.data)[k], names(omics)[i], "PCA"))
      #   ggsave(paste0(names(method.data)[k], "_", names(omics)[i], "_PCA_", temp.phenos[j], "_", Sys.Date(), "_batchCorrectedPatientCov_flowLabel.pdf"),width=5,height=5) # 3018 DIA (49%), 3975 (77%) TMT; wo outliers1: 3261 DIA (53%), 3986 TMT (77%), 17 TMT phospho (2%)
      # }
    }
  }
}
...
for (k in 1:length(method.data)) {
  ...
  # CD14Pos filter: Aza Sens vs Res: Error in panSEA2(contrasts, contrast2, meta.df, omics, annotations, gmt.list2 = gmt.list2,  : 
  # object 'dmea.results' not found
